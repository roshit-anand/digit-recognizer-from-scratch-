...
train_set_Y=train_Y.reshape(train_Y.shape[0],1)[:2000,:]
test_set_Y=train_Y.reshape(train_Y.shape[0],1)[2000:2500,:]

test_set_x_flatten = train_X.reshape(train_X.shape[0],-1).T[:,2000:2500]
train_set_x_flatten = train_X.reshape(train_X.shape[0],-1).T[:,:2000]

print(train_set_x_flatten.shape,train_Y.shape)
def onehotit(z):
...
def optimize(w,X,Y,learning_rate,num_iteration,t,costs):
...
w,costs,grad,cost,i=optimize(w,train_set_x_flatten,train_Y1,learning_rate,num_iteration,t,costs)
